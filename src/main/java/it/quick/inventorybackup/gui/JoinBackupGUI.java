...
public class JoinBackupGUI {

    private final InventoryBackup plugin;
    private final DatabaseManager databaseManager;

    public JoinBackupGUI(InventoryBackup plugin) {
        this.plugin = plugin;
        this.databaseManager = plugin.getDatabaseManager();
    }

    public void open(Player player) {
        int maxBackup = plugin.getConfigManager().getMaxBackup("Join");
        Inventory inventory = plugin.getServer().createInventory(null, 27, ChatColor.GOLD + "Backup Join");

        List<Backup> backups = databaseManager.getBackupsForJoin(player, maxBackup);
        ...
        for (Backup backup : backups) {
            ...
            if (meta != null) {
                meta.setDisplayName(ChatColor.GREEN + "Backup " + backup.getDateFormatted());
    ...
    public void openFullBackup(Player player, Backup backup) {
        Inventory inventory = Bukkit.createInventory(null, 54, ChatColor.GOLD + "Backup Join - " + backup.getDateFormatted());
